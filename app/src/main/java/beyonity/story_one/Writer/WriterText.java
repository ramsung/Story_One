...
package beyonity.story_one.Writer;
...
public class WriterText extends android.support.v7.widget.AppCompatEditText implements TextWatcher {
	...
	public WriterText(Context context) {
		super(context);
		init(null);
	}

	public WriterText(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(attrs);
	}

	public WriterText(Context context, AttributeSet attrs, int defStyleAttr) {
		super(context, attrs, defStyleAttr);
		init(attrs);
	}

    /*@SuppressWarnings("NewApi")
    public WriterText(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        super(context, attrs, defStyleAttr, defStyleRes);
        init(attrs);
    }*/

	private void init(AttributeSet attrs) {
		TypedArray array = getContext().obtainStyledAttributes(attrs, R.styleable.WriterText);
		bulletColor = array.getColor(R.styleable.WriterText_bulletColor, 0);
		bulletRadius = array.getDimensionPixelSize(R.styleable.WriterText_bulletRadius, 0);
		bulletGapWidth = array.getDimensionPixelSize(R.styleable.WriterText_bulletGapWidth, 0);
		historyEnable = array.getBoolean(R.styleable.WriterText_historyEnable, true);
		historySize = array.getInt(R.styleable.WriterText_historySize, 100);
		linkColor = array.getColor(R.styleable.WriterText_linkColor, 0);
		linkUnderline = array.getBoolean(R.styleable.WriterText_linkUnderline, true);
		quoteColor = array.getColor(R.styleable.WriterText_quoteColor, 0);
		quoteStripeWidth = array.getDimensionPixelSize(R.styleable.WriterText_quoteStripeWidth, 0);
		quoteGapWidth = array.getDimensionPixelSize(R.styleable.WriterText_quoteCapWidth, 0);
		array.recycle();
	...
	protected void styleInvalid(int style, int start, int end) {
		...
		List<WriterPart> list = new ArrayList<>();

		for (StyleSpan span : spans) {
			if (span.getStyle() == style) {
				list.add(new WriterPart(getEditableText().getSpanStart(span), getEditableText().getSpanEnd(span)));
				getEditableText().removeSpan(span);
			}
		}

		for (WriterPart part : list) {
			if (part.isValid()) {
	...
	protected void bulletValid() {
		...
		for (int i = 0; i < lines.length; i++) {
			...
			if (bulletStart < bulletEnd) {
				getEditableText().setSpan(new WriterBulletSpan(bulletColor, bulletRadius, bulletGapWidth), bulletStart, bulletEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
			}
		}
	}
	...
	protected void quoteValid() {
		...
		for (int i = 0; i < lines.length; i++) {
			...
			if (quoteStart < quoteEnd) {
				getEditableText().setSpan(new WriterQuoteSpan(quoteColor, quoteStripeWidth, quoteGapWidth), quoteStart, quoteEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
			}
		}
	}
	...
	// When WriterText lose focus, use this method
	...
	protected void linkValid(String link, int start, int end) {
		...
		linkInvalid(start, end);
		getEditableText().setSpan(new WriterURLSpan(link, linkColor, linkUnderline), start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
	}
	...
	public void fromHtml(String source) {
		...
		builder.append(beyonity.story_one.Writer.WriterParser.fromHtml(source));
		switchToWriterStyle(builder, 0, builder.length());
		setText(builder);
	}

	public String toHtml() {
		return WriterParser.toHtml(getEditableText());
	}

	protected void switchToWriterStyle(Editable editable, int start, int end) {
		...
		for (BulletSpan span : bulletSpans) {
			...
			editable.setSpan(new WriterBulletSpan(bulletColor, bulletRadius, bulletGapWidth), spanStart, spanEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
		}
		...
		for (QuoteSpan span : quoteSpans) {
			...
			spanEnd = 0 < spanEnd && spanEnd < editable.length() && editable.charAt(spanEnd) == '\n' ? spanEnd - 1 : spanEnd;
			editable.removeSpan(span);
			editable.setSpan(new WriterQuoteSpan(quoteColor, quoteStripeWidth, quoteGapWidth), spanStart, spanEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
		}
		...
		for (URLSpan span : urlSpans) {
			...
			int spanEnd = editable.getSpanEnd(span);
			editable.removeSpan(span);
			editable.setSpan(new WriterURLSpan(span.getURL(), linkColor, linkUnderline), spanStart, spanEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
		}
	}

	public void insertTab() {
		String c = "\t\t";
		int end = getSelectionStart() + c.length();
		String value1 = getEditableText().toString().substring(0, getSelectionStart());
		String value2 = getEditableText().toString().substring(getSelectionStart());
		StringBuilder builder = new StringBuilder();
		builder.append(value1);
		builder.append(c);
		builder.append(value2);
		getEditableText().clear();
		getEditableText().append(builder);
		setSelection(end);
	}
}
